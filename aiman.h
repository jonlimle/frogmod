...
namespace aiman {
	...
	void listbotnames() {
		if(botnames.length() > 0) {
			Wrapper w("");
			w.append("Listing bot names: ");
			loopv(botnames) { w.append("%s", botnames[i].name); w.sep = ", "; }
			loopv(w.lines) echo("%s", w.lines[i].s);
		} else echo("No bot names in list (all bots will be called \"bot\").");
	}
    ...
    void savebotnames(stream *s) {
    	loopv(botnames)
	    	s->printf("addbotname \"%s\"\n", botnames[i].name);
    }
